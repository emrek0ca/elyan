...
files = {
    'gizlilik': '/tmp/gizlilik.html',
    'kullanim-kosullari': '/tmp/kullanim-kosullari.html',
    'destek': '/tmp/destek.html',
    'yapay-zeka-bildirimi': '/tmp/yapay-zeka.html',
...
for slug, path in files.items():
    ...
    # Check if there is next.js string literal JSON that contains the content
    # Next.js Server Components serialize strings like "Gizlilik Politikası"
    # To be safe, we parse DOM. If DOM doesn't have it, we use regex.
    soup = bs4.BeautifulSoup(html, 'html.parser')
    
    hero = soup.find('section', class_='page-hero')
    if not hero:
        print(f"[{slug}] No hero section found. Falling back to regex.")
        # Fallback regex for next.js chunks
        title_match = re.search(r'<h1>(.*?)</h1>', html)
        title = title_match.group(1) if title_match else 'Sayfa'
        
        eyebrow_match = re.search(r'<span class=\\"eyebrow\\">(.*?)<', html) or re.search(r'class="eyebrow">(.*?)<', html)
        eyebrow = eyebrow_match.group(1) if eyebrow_match else 'Elyan'
        
        lede_match = re.search(r'<p class=\\"lede.*?\\">(.*?)<\/p>', html) or re.search(r'<p class="lede.*?">(.*?)<\/p>', html)
        lede = lede_match.group(1) if lede_match else ''
        
        content_html = ""
        # Find all h3 and their p's
        # Since it's stringified JSON, HTML tags might be escaped like <h3\> or \"h3\"
        # Best is to try to unescape the whole JSON chunk
        continue
    ...
    if content_section:
        ...
        print(f"[{slug}] No content section")
