...
class elyanUI:
    def __init__(self):
        ...
        self._elyan_state   = "INITIALISING"
        ...
        self._wave_elyan = [random.randint(4, 26) for _ in range(18)]
    ...
    def _build_social_bar(self):
        ...
        name_lbl.bind("<Button-1>", _open("https://www.instagram.com/osmanemrekoca_"))
        ...
        if self._icon_ig:
            ...
            ig_lbl.bind("<Button-1>", _open("https://www.instagram.com/osmanemrekoca_"))
    ...
    def _sync_sound_state(self):
        ...
        if enabled and self._elyan_state == "THINKING":
    ...
    def _toggle_pause(self):
        ...
        if self.paused:
            ...
            self.write_log("SYS: elyan duraklatıldı.")
        else:
            self.set_state("THINKING")
            self.write_log("SYS: elyan devam ediyor...")
        self._sync_sound_state()
    ...
    def _shutdown(self):
        self.sound.stop_all()
        self.write_log("SYS: elyan kapatılıyor...")
    ...
    def _on_input_submit(self, event=None):
        ...
        if not text:
            return
        if self.paused:
            self.write_log("SYS: elyan duraklatılmış durumda. Devam etmek için pause'u kapat.")
            return
        self._input_var.set("")
    ...
    def set_state(self, state: str):
        previous = getattr(self, "_elyan_state", "")
        self._elyan_state = state
    ...
    def _state_color(self, state: str | None = None) -> str:
        effective = state or self._elyan_state
        if effective == "PAUSED":
            return C_MID
    ...
    def _start_typing(self):
        if not self.typing_queue:
            self.is_typing = False
            if self._elyan_state == "ERROR" and time.time() < self._error_hold_until:
                return
        ...
        elif tl.startswith("elyan:") or tl.startswith("ai:"): tag = "ai"
    ...
    def _animate(self):
        ...
        if self.speaking and t % 3 == 0:
            self._wave_elyan = [random.randint(6, 30) for _ in range(18)]
        ...
        if now - self.last_t > (0.12 if self.speaking else 0.50):
            ...
            elif self._elyan_state in ("THINKING", "INITIALISING"):
    ...
    def _orb_rgb(self):
        state = "PAUSED" if self.paused else self._elyan_state
    ...
    def _draw_right_panel(self, c):
        ...
        if self.paused:
            ...
            sc, st = self._state_color(self._elyan_state), self._elyan_state
    ...
    def _draw(self):
        ...
        state_label = "PAUSED" if self.paused else self._elyan_state
        ...
        indicator_state = "PAUSED" if self.paused else self._elyan_state
        ...
        c.create_text(W//2, H-13, fill=C_DIM, font=font_body(9),
                      text="elyan · macOS Edition · Realtime Voice Core")
    ...
    def _save_api_key(self):
        ...
        if was_ready:
            ...
            self.set_state("LISTENING")
            self.write_log("SYS: elyan hazır. Dinliyorum...")
