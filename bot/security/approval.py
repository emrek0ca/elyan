...
class ApprovalManager:
    ...
    async def request_approval(self, operation: str, risk_level: RiskLevel,
        ...
        # FIX BUG-SEC-001: Clear stale requests for same user before adding new one
        async with self._lock:
            ...
            for stale_id in stale_ids:
                stale_req = self.pending_requests.pop(stale_id)
                stale_req.status = "expired"
                self.approval_history.append(stale_req)
                logger.warning(f"Cleared stale approval request {stale_id} for user {user_id}")
        ...
        try:
            ...
            approved = False
            logger.error(f"Approval callback error for {request_id}: {e}")
        finally:
            # FIX BUG-SEC-001: Always clean up pending request, even on exception
            async with self._lock:
                self.pending_requests.pop(request_id, None)
    ...
    def classify_operation_risk(self, operation: str, params: Dict[str, Any]) -> RiskLevel:
        ...
        if operation in high_risk_ops:
            if operation == "delete_file":
                ...
                if any(s in file_path.lower() for s in ["/system", "/library", ".app"]):
                    return RiskLevel.CRITICAL
                ...
                if any(d in command.lower() for d in ["rm -rf", "dd if=", "mkfs"]):
                    return RiskLevel.CRITICAL
                return RiskLevel.HIGH
            return RiskLevel.HIGH
...
# Global approval manager (thread-safe singleton via module-level lock)
_approval_manager: Optional[ApprovalManager] = None
_approval_manager_lock = asyncio.Lock()


def get_approval_manager() -> ApprovalManager:
    """Get or create global approval manager (sync-safe for module init)"""
