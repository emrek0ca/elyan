...
class ElyanGatewayServer:
    ...
    async def handle_create_task(self, request):
        ...
        if self._looks_like_automation_request(text):
            try:
    ...
    def _looks_like_automation_request(text: str) -> bool:
        ...
        if not low:
            return False
        schedule_markers = (
            "her gün", "hergun", "günlük", "gunluk", "daily",
            "hafta içi", "haftaici", "haftalık", "haftalik", "weekly",
            "saat", "cron", "her pazartesi", "her salı", "her sali", "her cuma",
        )
        action_markers = (
            "rutin", "otomasyon", "kontrol et", "rapor", "gönder", "gonder",
            "panel", "tarayıcı", "tarayici", "sipariş", "siparis", "stok",
            "mail", "muhasebe", "öğrenci", "ogrenci",
        )
        has_schedule = any(m in low for m in schedule_markers)
        has_action = any(m in low for m in action_markers)
        return has_schedule and has_action
