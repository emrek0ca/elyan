...
echo "==> Bootstrapping subscription lifecycle schema (pending plan columns)"
docker compose -f "${COMPOSE_FILE}" exec -T "${POSTGRES_SERVICE}" \
  psql -v ON_ERROR_STOP=1 -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" \
  < drizzle/0027_subscription_pending_plan.sql

echo "==> Subscription lifecycle schema bootstrap complete"
