...
class _LogoRequestError(RuntimeError):
    pass


class _LogoCredentialError(_LogoRequestError):
    pass
...
class LogoConfig:
    ...
    def endpoint_url(self, path: str = "") -> str:
        ...
        if not base_url:
            raise _LogoRequestError("Logo API base URL tanimli degil")
...
class LogoConnector(ConnectorBase):
    def __init__(
        self,
        *,
        config: LogoConfig | None = None,
        credentials: LogoCredentials | dict[str, Any] | None = None,
    ...
    @staticmethod
    def _coerce_credentials(credentials: LogoCredentials | dict[str, Any] | None) -> LogoCredentials:
        if isinstance(credentials, LogoCredentials):
            return credentials
        if isinstance(credentials, dict):
            return LogoCredentials(
    ...
    def health_check(self) -> ConnectorHealth:
        ...
        except _LogoRequestError as exc:
    ...
    def test_credentials(self) -> bool:
        if not self.credentials.is_configured():
            self._audit(
                ...
                success=False,
                duration_ms=0.0,
                result={"error": "Logo kimlik bilgileri eksik"},
            )
            return False
    ...
    def _consent_error(self) -> str | None:
        consent = self.privacy_engine.get_consent(
            ...
            scope="turkey_connector.logo",
        )
        if bool(consent.get("granted")):
            return None
        return "KVKK onayi olmadan Logo kimlik dogrulamasi yapilamaz"
        ...
        for attempt in range(1, self.max_attempts + 1):
            ...
            try:
                ...
                if response.status_code in {401, 403}:
                    error = "Logo kimlik dogrulamasi reddedildi"
                    self._audit(
                        action=action,
                        success=False,
                        duration_ms=latency_ms,
                    ...
                    raise _LogoCredentialError(error)
                error = f"Logo servisi beklenmeyen durum kodu dondurdu: {response.status_code}"
                self._audit(
                    action=action,
                    success=False,
                    duration_ms=latency_ms,
                ...
                if attempt >= self.max_attempts:
                    raise _LogoRequestError(error)
            except httpx.HTTPError as exc:
                latency_ms = round((time.perf_counter() - started_at) * 1000.0, 2)
                error = f"Logo servisine baglanilamadi: {exc}"
                self._audit(
                    action=action,
                    success=False,
                    duration_ms=latency_ms,
                ...
                if attempt >= self.max_attempts:
                    raise _LogoRequestError(error) from exc
            if attempt < self.max_attempts:
                self._sleeper(self.retry_backoff_seconds * (2 ** (attempt - 1)))
        raise _LogoRequestError("Logo istegi sonucsuz kaldi")
    ...
    def _audit(
        ...
        *,
        ...
        self.audit_logger.log_operation(
            user_id=0,
            operation="turkey_connector.logo",
            action=action,
            params={
