...
from integrations.turkey.netsis import NetsisConfig, NetsisConnector, NetsisCredentials
...
def test_health_check_uses_test_endpoint(tmp_path) -> None:
    ...
    session = _FakeSession(
        [httpx.Response(200, request=httpx.Request("GET", "https://netsis-test.example.com/health"))]
    )
    connector = NetsisConnector(
        config=NetsisConfig(
            production_base_url="https://netsis-prod.example.com",
            test_base_url="https://netsis-test.example.com",
            health_path="health",
        ),
        session=session,
    ...
    assert session.calls[0]["url"] == "https://netsis-test.example.com/health"
...
def test_test_credentials_requires_consent(tmp_path) -> None:
    ...
    connector = NetsisConnector(
        ...
        credentials=NetsisCredentials(username="demo", password="secret"),
        session=session,
        audit_logger=audit_logger,
        privacy_engine=PrivacyEngine(db_path=tmp_path / "consent.db", runtime_db_path=tmp_path / "runtime.sqlite3"),
        user_id="user-1",
...
def test_test_credentials_retries_with_backoff(tmp_path) -> None:
    ...
    session = _FakeSession(
        [
            ...
            httpx.Response(200, request=httpx.Request("GET", "https://netsis-test.example.com/auth/check")),
        ]
    )
