...
from integrations.turkey.sgk import SGKConfig, SGKConnector, SGKCredentials
...
def test_health_check_uses_test_endpoint(tmp_path) -> None:
    ...
    session = _FakeSession(
        [httpx.Response(200, request=httpx.Request("GET", "https://sgk-test.example.com/health"))]
    )
    connector = SGKConnector(
        config=SGKConfig(
            production_base_url="https://sgk-prod.example.com",
            test_base_url="https://sgk-test.example.com",
            health_path="health",
        ),
        session=session,
    ...
    assert session.calls[0]["url"] == "https://sgk-test.example.com/health"
...
def test_test_credentials_requires_consent(tmp_path) -> None:
    ...
    connector = SGKConnector(
        ...
        credentials=SGKCredentials(username="demo", password="secret"),
        session=session,
        audit_logger=audit_logger,
        privacy_engine=PrivacyEngine(db_path=tmp_path / "consent.db", runtime_db_path=tmp_path / "runtime.sqlite3"),
        user_id="user-1",
...
def test_test_credentials_retries_with_backoff(tmp_path) -> None:
    ...
    session = _FakeSession(
        [
            ...
            httpx.Response(200, request=httpx.Request("GET", "https://sgk-test.example.com/auth/check")),
        ]
    )
