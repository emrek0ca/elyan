...
Napi::Object DesktopCapabilitySnapshot(const Napi::CallbackInfo& info) {
  ...
  permissions.Get("accessibility").As<Napi::Object>().Set("required", Napi::Boolean::New(env, true));
  permissions.Get("accessibility").As<Napi::Object>().Set("granted", env.Null());
  permissions.Get("accessibility").As<Napi::Object>().Set("status", Napi::String::New(env, "required"));
  ...
  result.Set("activeWindow", elyan::native::BuildActiveWindowInfo(env));
  return result;
}
