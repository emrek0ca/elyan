...
import type { BootstrapSnapshot, SystemCapabilities } from '../../shared/protocol';
...
type SettingsSection = 'account' | 'local' | 'cloud' | 'pairing' | 'privacy' | 'advanced';
...
function ListSection({ title, description, children }: { title: string; description?: string; children: ReactNode }) {
  return (
...
export function SettingsSurface({
  ...
  const runtimeReady = runtimeReadiness.canReceiveTasks === true;
  ...
  async function loadAvatar() {
    ...
    if (dataBase64) {
      setAvatarPreview(`data:${mimeType};base64,${dataBase64}`);
    }
  }
  ...
  async function uploadAvatar(file: File | null) {
    ...
    await withBusy('avatar-upload', async () => {
      ...
      const response = await desktopApi.request({
        capability: 'backend.auth_avatar_upload',
        payload,
      });
      if (!response.ok) {
        throw new Error(response.error?.message ?? 'Fotoğraf yüklenemedi.');
      }
      await Promise.resolve(onRefresh());
      await loadAvatar();
  ...
  async function removeAvatar() {
    await withBusy('avatar-delete', async () => {
      const response = await desktopApi.request({ capability: 'backend.auth_avatar_delete', payload: {} });
      if (!response.ok) {
        throw new Error(response.error?.message ?? 'Fotoğraf kaldırılamadı.');
      }
      setAvatarPreview('');
      await Promise.resolve(onRefresh());
  ...
  async function saveDangerousAreaEnabled(nextValue: boolean) {
    await withBusy('privacy-dangerous-area', async () => {
      const response = await desktopApi.request({
        capability: 'state.update',
        payload: {
          account: {
            dangerousAreaEnabled: nextValue,
          },
        },
      });
      if (!response.ok) {
  ...
  const sections: Array<{ id: SettingsSection; label: string; icon: ReactNode }> = [
    ...
    { id: 'pairing', label: 'Telefon Bağlantısı', icon: <IconPhone /> },
        ...
        <section className="settings-panel">
          ...
          {activeSection === 'local' ? (
            <div className="settings-section">
              <ListSection title="Yerel modeller" description="Bilgisayarındaki modelleri burada seçip kullanabilirsin.">
                <ListItem
          ...
          {activeSection === 'cloud' ? (
            <div className="settings-section">
              <ListSection title="Bulut modeller" description="Bağlantı anahtarını ekleyip kullanmaya başlayabilirsin.">
                    ...
                    >
                      {isExpanded ? (
                        <div className="settings-inline-editor">
                          ...
                          <InlineField label="Bağlantı adresi" value={form.baseUrl} placeholder="İsteğe bağlı" onChange={(value) => updateProviderForm(providerId, { baseUrl: value, enabled: true })} />
                          <div className="settings-inline-actions">
          ...
          {activeSection === 'account' ? (
            <div className="settings-section">
              <ListSection title="Hesap" description="Kişisel bilgilerini ve giriş ayarlarını buradan yönetebilirsin.">
                <ListItem
                  title={displayName}
                  hint={email || 'E-posta eklenmemiş.'}
                  status=""
                  actions={
          ...
          {activeSection === 'pairing' ? (
            <div className="settings-section">
              <ListSection title="Telefon bağlantısı" description="Telefonundaki Elyan ile bu bilgisayarı burada eşleştirebilirsin.">
                <ListItem
                  title="Bağlantı durumu"
                  hint={runtimeReady ? 'Telefon bu bilgisayara görev gönderebilir.' : 'Bağlantıyı yenileyerek hazır hale getirebilirsin.'}
                  status={runtimeReady ? 'Hazır' : 'Bekliyor'}
                  actions={
                    <>
                  ...
                  <div className="settings-pairing-detail">
                    ...
                    <div className="settings-pairing-rows">
                      ...
                      <div>
                        ...
                        <span>{runtimeReady ? 'Görev almaya hazır' : 'Bağlantı bekleniyor'}</span>
                      </div>
                      <div>
                        <strong>Telefonla eşleşme</strong>
                        <span>{asString(pairing.lastSessionStatus, 'Bekleniyor')}</span>
                      </div>
                      <div>
          ...
          {activeSection === 'privacy' ? (
            <div className="settings-section">
              <ListSection title="Gizlilik" description="Kişisel verilerin nasıl işlendiğini burada görebilirsin.">
                <ListItem
                  title="Yerel çalışma"
                  hint="Özel işler mümkün olduğunda bu bilgisayarda tutulur."
                ...
                <ListItem
                  ...
                  hint="Ürünü geliştirmek için anonim kalite sinyalleri gönderilebilir."
                ...
                <ListItem
                  ...
                  hint="Hata kayıtlarında özel içerik mümkün olduğunca maskelenir."
                ...
                <ListItem
                  title="İzin geçmişi"
                  hint="Hangi izinlerin ne zaman kullanıldığı bu bilgisayarda tutulur."
                ...
                />
                <ListItem
                  title="Onay gerektiren güçlü işlemler"
                  hint="Gönderme, yazma ve benzeri güçlü işlemler için yerel onay katmanı kullanılır."
                  ...
                  actions={
                    <label className="settings-inline-toggle">
                      <input
                        ...
                        onChange={(event) => void saveDangerousAreaEnabled(event.currentTarget.checked)}
                      />
                      <span>Kullan</span>
                    </label>
                  }
                />
                <ListItem title="Dosya kullanımı" hint="Kendi dosyalarına erişim izni olmadan işlem yapılmaz." status={boolLabel(asBoolean(asRecord(stateRecord(snapshot).permissions).allow_file_indexing))} />
                <ListItem title="Sistem izinleri" hint="Ekran ve bilgisayar izinleri gerektiğinde ayrı ayrı sorulur." status={asString(systemCapabilities?.nativeControl.osPermissionModel, 'Hazır değil')} />
                <ListItem
                  title="Hesabı sil"
          ...
          {activeSection === 'advanced' ? (
            <div className="settings-section">
              <ListSection title="Gelişmiş" description="Daha özel bağlantıları ve genel davranışı buradan ayarlayabilirsin.">
                <ListItem
                  title="Bulut yedeği"
                ...
                {advancedProviderItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {isExpanded ? (
                        <div className="settings-inline-editor">
                          <InlineField label="Bağlantı adresi" value={form.baseUrl} onChange={(value) => updateProviderForm(providerId, { baseUrl: value, enabled: true })} />
