...
const strategies: CapabilityLibraryStrategy[] = [
  ...
  },
  {
    ...
    libraries: [
      { name: 'ts-morph', status: 'planned', purpose: 'TypeScript AST inspection and safe structural edits.' },
      { name: 'diff', status: 'planned', purpose: 'Human-readable patch previews and verification summaries.' },
      { name: 'simple-git', status: 'planned', purpose: 'Git status, branch, and history access without shell parsing.' },
      { name: 'ignore', status: 'planned', purpose: 'Respect gitignore-style boundaries during file discovery.' },
      { name: 'fast-glob', status: 'planned', purpose: 'Fast bounded workspace search across allowed roots.' },
    ],
