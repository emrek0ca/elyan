...
export async function POST(
  ...
  try {
    ...
    const params = draftParamsSchema.safeParse(await context.params);

    if (!params.success) {
      return NextResponse.json(
        { ok: false, error: 'Invalid draft parameters', issues: params.error.flatten().fieldErrors },
        { status: 400 }
      );
    }

    assertControlPlaneAccountAccess(session, session.accountId!);
    const result = await getControlPlaneService().promoteLearningDraft(session.accountId!, params.data.draftId);
