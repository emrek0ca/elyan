...
const RESTRICTED_TEXT_PATTERN =
  /(?<!\p{L})(şifre|sifre|parola|password|otp|doğrulama kodu|dogrulama kodu|kredi kartı|kredi karti|cvv|kimlik no|tc kimlik|pasaport|passport|api key|secret key|private key|seed phrase|contraseña|contrasena|código de verificación|codigo de verificacion|tarjeta de crédito|tarjeta de credito|mot de passe|code de vérification|code de verification|carte de crédit|carte de credit|kennwort|passwort|bestätigungscode|bestatigungscode|kreditkarte|passaporto|senha|cartão de crédito|cartao de credito|пароль|код подтверждения|кредитная карта|паспорт|كلمة المرور|رمز التحقق|بطاقة ائتمان|جواز سفر)(?!\p{L})/iu;
