...
const TASK_RULES: Array<{ task: VisionTask; pattern: RegExp }> = [
  ...
  { task: "screen_debugging", pattern: /(?<!\p{L})(ekran görüntü\p{L}*|ekran goruntu\p{L}*|screenshot|uygulama ekranı|uygulama ekrani|arayüz|arayuz|buton|ui|ux|pantalla|interfaz|capture d['’]écran|écran|ecran|oberfläche|oberflache|bildschirm|schermata|schermo|tela|интерфейс|экран|واجهة|شاشة)(?!\p{L})/iu },
  { task: "code_screenshot", pattern: /(?<!\p{L})(kod|code|stack trace|terminal|console|exception|hata|hata mesajı|hata mesaji|debug|mensaje de error|erreur|fehlermeldung|messaggio di errore|mensagem de erro|ошибка|сообщение об ошибке|رسالة خطأ|خطأ)(?!\p{L})/iu },
  ...
  { task: "chart_interpretation", pattern: /(?<!\p{L})(grafik|grafiğ\p{L}*|chart|trend|eksen|axis|seri|dağılım|dagilim|gráfico|grafico|tendencia|eje|graphique|tendance|axe|diagramm|achse|grafico|tendenza|gráfico|tendência|tendencia|график|тенденция|ось|مخطط|اتجاه|محور)(?!\p{L})/iu },
  { task: "handwriting", pattern: /(?<!\p{L})(el yaz\p{L}*|handwriting|not defteri|defterde|escritura a mano|écriture manuscrite|ecriture manuscrite|handschrift|scrittura a mano|escrita à mão|escrita a mao|рукописный|почерк|خط اليد)(?!\p{L})/iu },
...
const CODE_SPECIFIC_PATTERN = /(?<!\p{L})(flutter|stack trace|terminal|console|exception|hata kod\p{L}*|hata mesaj\p{L}*|error code|error message|mensaje de error|fehlermeldung|messaggio di errore|mensagem de erro|сообщение об ошибке|رسالة خطأ)(?!\p{L})/iu;
...
const TURKISH_SUFFIXES = [
  "larındaki", "lerindeki", "sındaki", "sindeki", "sundaki", "sündeki",
  "ındaki", "indeki", "undaki", "ündeki", "daki", "deki", "taki", "teki",
  "larını", "lerini", "sını", "sini", "sunu", "sünü",
  "ın", "in", "un", "ün", "nın", "nin", "nun", "nün",
  "lar", "ler", "dan", "den", "tan", "ten", "da", "de", "ta", "te",
] as const;

function expandTurkishSearchForms(prompt: string): string {
  ...
  for (const token of normalized.split(/[^\p{L}\p{N}]+/u).filter(Boolean)) {
    ...
    for (let pass = 0; pass < 2; pass += 1) {
      const suffix = TURKISH_SUFFIXES.find(
        (candidate) => current.length - candidate.length >= 3 && current.endsWith(candidate),
      );
      if (!suffix) break;
...
export function classifyVisionTask(input: {
  ...
  const searchablePrompt = expandTurkishSearchForms(prompt);
  const matched = TASK_RULES.filter((rule) => rule.pattern.test(searchablePrompt)).map((rule) => rule.task);
  if (matched.includes("screen_debugging") && matched.includes("code_screenshot") && CODE_SPECIFIC_PATTERN.test(searchablePrompt)) {
    matched.splice(matched.indexOf("code_screenshot"), 1);
    matched.unshift("code_screenshot");
  }
  if ((input.imageCount ?? 0) > 1 && !matched.includes("visual_comparison")) {
    if (matched.length === 0) matched.unshift("visual_comparison");
    else matched.push("visual_comparison");
  }
