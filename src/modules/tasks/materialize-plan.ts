...
async function repairUnusableMaterializedPlan(
  ...
  try {
    ...
    const prompt = [
      ...
      "- READ-ONLY PRECEDENCE: if the user only asks to observe, list, find, check, read, or report a state and explicitly forbids changing, deleting, writing, or saving, treat conflicting artifact/document-writer metadata as stale; use only read capabilities and never add a writer or artifact step.",
