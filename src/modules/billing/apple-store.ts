import { readFile } from "node:fs/promises";
import {
  Environment,
  SignedDataVerifier,
  type JWSTransactionDecodedPayload,
  type JWSRenewalInfoDecodedPayload,
  type ResponseBodyV2DecodedPayload,
} from "@apple/app-store-server-library";
import { decodeJwt } from "jose";
import type { AppEnv } from "../../config/env.js";
import { conflict } from "../../lib/errors.js";

// Public Apple trust anchors from https://www.apple.com/certificateauthority/.
const APPLE_ROOT_CERTIFICATES = [
  "",
  "",
  "",
].map((certificate) => Buffer.from(certificate, "base64"));

type AppleEnvironment = Environment.SANDBOX | Environment.PRODUCTION;

function environmentCandidates(signedPayload: string): AppleEnvironment[] {
  try {
    const environment = String(decodeJwt(signedPayload).environment || "").toLowerCase();
    if (environment === "sandbox") {
      return [Environment.SANDBOX, Environment.PRODUCTION];
    }
  } catch {
    // This unverified decode is only an environment hint.
  }
  return [Environment.PRODUCTION, Environment.SANDBOX];
}

function createVerifier(config: AppEnv, environment: AppleEnvironment): SignedDataVerifier {
  const bundleId = config.APPLE_APP_BUNDLE_ID.trim();
  const appAppleId = Number(config.APPLE_APP_ID);
  if (!bundleId) {
    throw conflict("apple_app_bundle_id_missing");
  }
  if (environment === Environment.PRODUCTION && (!Number.isSafeInteger(appAppleId) || appAppleId <= 0)) {
    throw conflict("apple_numeric_app_id_missing");
  }
  return new SignedDataVerifier(
    APPLE_ROOT_CERTIFICATES,
    true,
    environment,
    bundleId,
    environment === Environment.PRODUCTION ? appAppleId : undefined,
  );
}

async function verifyWithEnvironment<T>(
  config: AppEnv,
  signedPayload: string,
  operation: (verifier: SignedDataVerifier) => Promise<T>,
): Promise<{ environment: AppleEnvironment; decoded: T }> {
  let lastError: unknown;
  for (const environment of environmentCandidates(signedPayload)) {
    try {
      return {
        environment,
        decoded: await operation(createVerifier(config, environment)),
      };
    } catch (error) {
      lastError = error;
    }
  }
  throw conflict("apple_signed_data_verification_failed", {
    reason: lastError instanceof Error ? lastError.name : "verification_failed",
  });
}

export async function verifyAppleTransaction(
  config: AppEnv,
  signedTransaction: string,
): Promise<{ environment: AppleEnvironment; transaction: JWSTransactionDecodedPayload }> {
  const verified = await verifyWithEnvironment(
    config,
    signedTransaction,
    (verifier) => verifier.verifyAndDecodeTransaction(signedTransaction),
  );
  return { environment: verified.environment, transaction: verified.decoded };
}

export async function verifyAppleNotification(
  config: AppEnv,
  signedPayload: string,
): Promise<{
  environment: AppleEnvironment;
  notification: ResponseBodyV2DecodedPayload;
  transaction: JWSTransactionDecodedPayload | null;
  renewalInfo: JWSRenewalInfoDecodedPayload | null;
}> {
  const verified = await verifyWithEnvironment(
    config,
    signedPayload,
    (verifier) => verifier.verifyAndDecodeNotification(signedPayload),
  );
  const signedTransaction = verified.decoded.data?.signedTransactionInfo;
  const transaction = signedTransaction
    ? await createVerifier(config, verified.environment).verifyAndDecodeTransaction(signedTransaction)
    : null;
  const signedRenewalInfo = verified.decoded.data?.signedRenewalInfo;
  const renewalInfo = signedRenewalInfo
    ? await createVerifier(config, verified.environment).verifyAndDecodeRenewalInfo(signedRenewalInfo)
    : null;
  return {
    environment: verified.environment,
    notification: verified.decoded,
    transaction,
    renewalInfo,
  };
}

export async function readApplePrivateKey(config: AppEnv): Promise<string> {
  const inlineKey = config.APPLE_APP_STORE_PRIVATE_KEY.trim().replace(/\\n/g, "\n");
  if (inlineKey) {
    return inlineKey;
  }
  const keyPath = config.APPLE_APP_STORE_PRIVATE_KEY_PATH.trim();
  if (!keyPath) {
    throw conflict("apple_app_store_private_key_missing");
  }
  return readFile(keyPath, "utf8");
}
