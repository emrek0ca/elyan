...
export class WindowManager extends EventEmitter {
  ...
  createOrFocus(): BrowserWindow {
    ...
    const browserWindow = new BrowserWindow({
      ...
      webPreferences: {
        ...
        // The preload bridge imports local compiled modules. Keep renderer Node
        // access disabled, but allow the preload's Node environment to load
        // the typed desktop API instead of falling back to preview mode.
        sandbox: false,
        spellcheck: false,
      },
    });
    ...
    this.bindExternalNavigation(browserWindow);
  ...
  private bindExternalNavigation(browserWindow: BrowserWindow): void {
    browserWindow.webContents.setWindowOpenHandler(({ url }) => {
      this.openExternalHttpUrl(url);
      return { action: 'deny' };
    });
    ...
    browserWindow.webContents.on('will-navigate', (event, url) => {
      ...
      if (this.openExternalHttpUrl(url)) {
        event.preventDefault();
      }
    });
  }

  private openExternalHttpUrl(rawUrl: string): boolean {
    try {
