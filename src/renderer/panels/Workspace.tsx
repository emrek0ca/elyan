    ...
    <main className={`workspace ${isComposerFocused ? 'workspace--composer-focused' : ''}`} aria-label="Elyan chat">
      <div className="workspace-topbar">
        ...
        <button
          ...
          aria-label="Mobil eşleştirme"
          onClick={() => setShowMobileBubble((v) => !v)}
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.1" strokeLinecap="round" strokeLinejoin="round">
            <rect x="6" y="2" width="12" height="20" rx="3.5" ry="3.5"></rect>
        ...
        {showMobileBubble ? (
          <div className="mobile-bubble">
            <div className="mobile-bubble-content">
              {showQR ? (
                <>
                  <h4>{mobileConnectionCopy.title}</h4>
                  <p>{mobileConnectionCopy.detail}</p>
                  <div className="mobile-bubble-qr">
                    {qrDataUrl ? <img src={qrDataUrl} alt="Telefon eşleştirme QR" /> : <div className="mobile-bubble-qr-skeleton"></div>}
                  </div>
                </>
              ) : (
                <>
              ...
              <div className="mobile-bubble-actions">
                <button type="button" className="button-secondary" onClick={() => setShowMobileBubble(false)}>
                  {showQR ? "Kapat" : "Anladım"}
                </button>
                {!showQR && !isMobileConnected && (
            ...
            <svg className="mobile-bubble-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="0.75" strokeLinecap="round" strokeLinejoin="round">
      ...
      <section className={`desktop-task-shell desktop-task-shell--${taskShell.readinessTone}`} aria-label="Desktop görev durumu">
        <button type="button" className="desktop-task-shell__main" onClick={onOpenTasks}>
          <span className={`desktop-task-shell__dot desktop-task-shell__dot--${taskShell.readinessToken}`} aria-hidden="true" />
          <span>
            <strong>{taskShell.desktopHeadline}</strong>
            <small>{taskShell.connectionSummary || taskShell.readinessDetail}</small>
          </span>
        </button>
        <div className="desktop-task-shell__metrics" aria-label="Runtime yetenekleri">
          <span title={taskShell.modelLabel}>{taskShell.modelLabel}</span>
          <span title={taskShell.learningLabel}>{taskShell.learningLabel}</span>
          <span title={taskShell.retrievalLabel}>{taskShell.retrievalLabel}</span>
          <span>{taskShell.powerCapabilityCount} yetenek</span>
          {taskShell.blockedCapabilityCount > 0 ? <span>{taskShell.blockedCapabilityCount} bloke</span> : null}
        </div>
        <div className="desktop-task-shell__actions">
          <button type="button" className="button-secondary" onClick={onOpenTasks}>
            Görevler
          </button>
          <button
            type="button"
            className="button-primary"
            disabled={!taskShell.canExecuteAssignedTasks || busy}
            onClick={onExecuteAssignedTasks}
          >
            Kuyruğu Al
          </button>
        </div>
      ...
      {taskShell.approvalTasks.length > 0 ? (
        <section className="desktop-approval-card" aria-label="Onay bekleyen desktop görevleri">
          <div className="desktop-approval-card__header">
            <span>Onay bekliyor</span>
            <button type="button" onClick={onOpenTasks}>Tümünü gör</button>
          </div>
          {taskShell.approvalTasks.map((task) => (
            <article key={task.id} className="desktop-approval-card__item">
              <span className="desktop-approval-card__status">{task.statusLabel}</span>
              <strong>{task.title}</strong>
              <p>{task.summary || task.approvalKind || 'Bu görev desktop tarafında yan etki yaratmadan önce onay bekliyor.'}</p>
              <div className="desktop-approval-card__meta">
                {task.route ? <span>{task.route}</span> : null}
                {task.approvalKind ? <span>{task.approvalKind}</span> : null}
                {task.artifactCount > 0 ? <span>{task.artifactCount} artifact</span> : null}
              </div>
              <div className="desktop-approval-card__actions">
                <button type="button" className="button-secondary" disabled={busy} onClick={() => onApproveTask(task.id, false)}>
                  Reddet
                </button>
                <button type="button" className="button-primary" disabled={busy} onClick={() => onApproveTask(task.id, true)}>
                  Onayla
                </button>
              </div>
            </article>
