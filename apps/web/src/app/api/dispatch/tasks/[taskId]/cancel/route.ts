...
import { cancelDispatchTask } from '@/core/dispatch';
...
export async function POST(request: NextRequest, context: { params: Promise<{ taskId: string }> }) {
  try {
    ...
    const task = await cancelDispatchTask(taskId, note);
    ...
    const normalized = normalizeApiError(error, {
      status: 500,
      code: 'dispatch_task_cancel_failed',
      message: 'Dispatch task cancel failed',
    });
