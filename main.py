#!/usr/bin/env python3
"""
elyan macOS — Gercek zamanli sesli yardimci cekirdegi
...
def load_system_prompt() -> str:
    try:
        ...
        return (
            "Sen elyan'sin — macOS'ta çalışan kişisel AI asistanı. "
...
class elyanLive:
    ...
    def _on_text_command(self, text: str):
        ...
        if not self._loop or not self.session:
            self.ui.write_log("ERR: elyan bağlantısı henüz hazır değil.")
            return
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[elyan] 📤 {name} → {str(result)[:80]}")
    ...
    async def _listen_audio(self):
        ...
        try:
            while True:
                ...
                with self._speaking_lock:
                    elyan_speaking = self._is_speaking
                if not elyan_speaking and not self.ui.muted and not self._paused:
                    await self.out_queue.put({"data": data, "mime_type": "audio/pcm"})
        except Exception as e:
            print(f"[elyan] ❌ Mikrofon: {e}")
            raise
        finally:
            stream.close()

    async def _receive_audio(self):
        print("[elyan] 👂 Alım başladı")
        out_buf, in_buf = [], []
        output_noise = False
        ...
        try:
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                self.ui.write_log(f"elyan: {full_out}")
                                ...
                                self.ui.write_log("ERR: elyan sesli yanıtını çözümlerken bir hata oluştu.")
                                if output_noise_samples:
                                    self.ui.write_debug(
            ...
            print(f"[elyan] ❌ Alım: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[elyan] 🔊 Ses çalma başladı")
        stream = await asyncio.to_thread(
            pya.open,
            format=FORMAT, channels=CHANNELS,
        ...
        while True:
            ...
            try:
                ...
                async with (
                    ...
                    print("[elyan] ✅ Bağlandı.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: elyan hazır. Dinliyorum...")
                ...
                traceback.print_exc()
                self.set_speaking(False)
                self.ui.write_log(f"ERR: elyan baglantisi kesildi veya internete ulasilamiyor — {e}")
                self.ui.set_state("ERROR")
                print("[elyan] 🔄 3 saniyede yeniden bağlanıyor...")
                await asyncio.sleep(3)


def main():
    if os.environ.get("TERM_PROGRAM") == "vscode":
        print("[elyan] VS Code icinden baslatildi.")
