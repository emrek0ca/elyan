...
class TaskEngine:
    ...
    async def _decompose_tasks(
        ...
        key_tools = [
            ...
            "read_word(path)", "read_pdf(path)", "analyze_pdf_vision(path)", "read_excel(path)",
            "create_smart_file(type,title,content,path)",
            "smart_summarize(text|path)", "analyze_document(path)",
            "analyze_document_vision(path)", "extract_tables_from_document(path)",
            "extract_charts_from_document(path)",
            "summarize_document_rag(path|text,style)", "document_rag_qa(path|text,question)",
            "build_document_rag_index(paths)", "analyze_document_rag(path)",
            ...
            "generate_document_pack(topic,brief,audience,language,output_dir,preferred_formats)",
            "research_document_delivery(topic,brief,depth,audience,language,output_dir,include_presentation)",
