...
HELPER_SOURCE = HELPERS_DIR / "elyan_calendar_helper.swift"
HELPER_PLIST = HELPERS_DIR / "elyan_calendar_helper.plist"
HELPER_APP = HELPERS_DIR / "elyan Calendar Helper.app"
...
HELPER_BIN = HELPER_MACOS_DIR / "elyan-calendar-helper"
...
def _run_helper(mode: str, payload: dict | None = None, timeout: int = 20) -> tuple[bool, str]:
    ...
    raw = ""
    try:
        handle = tempfile.NamedTemporaryFile(prefix="elyan-calendar-", suffix=".json", delete=False)
        output_path = Path(handle.name)
        handle.close()
        helper_args = [mode]
        if payload is not None:
            payload_handle = tempfile.NamedTemporaryFile(prefix="elyan-helper-payload-", suffix=".json", delete=False)
...
def _raise_calendar_error(detail: str, *, prefix: str) -> None:
    safe_detail = str(detail or "").strip()
    if is_permission_detail(safe_detail):
        raise permission_required(_calendar_permission_message())
    if is_timeout_detail(safe_detail):
        raise timeout_error(f"{prefix} zaman asimina ugradi.")
    if safe_detail:
        raise capability_unavailable(safe_detail)
    raise capability_unavailable(prefix)
...
def _calendar_permission_message() -> str:
    return (
        ...
        "'elyan Calendar Helper' uygulamasini ara ve izin ver."
    )
...
def get_calendar_events(query: str = "today", limit: int = 6) -> str:
    ...
    if not ok:
        _raise_calendar_error(raw, prefix="Takvim okunamadi")

    parsed_ok, detail, events = _parse_payload(raw)
    if not parsed_ok:
        _raise_calendar_error(detail, prefix="Takvim okunamadi")
...
def add_calendar_event(
    ...
    if not title:
        raise invalid_argument("Takvime eklemek icin etkinlik basligi gerekli.")
    if not start_iso:
        raise invalid_argument("Takvime eklemek icin baslangic tarihi gerekli.")
    ...
    if not ok:
        _raise_calendar_error(raw, prefix="Takvim etkinligi eklenemedi")

    parsed_ok, detail, event = _parse_single_event_payload(raw)
    if not parsed_ok:
        _raise_calendar_error(detail, prefix="Takvim etkinligi eklenemedi")
...
def delete_calendar_event(
    ...
    title = (title or "").strip()
    if not title:
        raise invalid_argument("Takvimden silmek icin etkinlik basligi gerekli.")
    ...
    if not ok:
        _raise_calendar_error(raw, prefix="Takvim etkinligi silinemedi")

    parsed_ok, detail, event = _parse_deleted_event_payload(raw)
    if not parsed_ok:
        _raise_calendar_error(detail, prefix="Takvim etkinligi silinemedi")
