...
def save_whatsapp_contact(display_name: str, phone_number: str, aliases: str = "") -> str:
    if not display_name or not display_name.strip():
        raise invalid_argument("Kişi adı boş olamaz.")
...
def _open_in_browser(url: str) -> str:
    if not is_macos():
        try:
            ...
            raise capability_unavailable("WhatsApp Web guvenli sekilde acilamadi.") from exc
        raise capability_unavailable("WhatsApp Web guvenli sekilde acilamadi.")
...
def send_whatsapp_message(
    ...
    if not message or not message.strip():
        raise invalid_argument("Mesaj bos olamaz.")
    ...
    if not is_macos():
        if send_now:
            raise unsupported_platform("WhatsApp otomatik gonderim su anda yalnizca macOS'ta destekleniyor.")
        if not normalized_phone:
            if resolved_name:
                raise unsupported_platform(
                    "Bu platformda kisi adina gore WhatsApp masaustu otomasyonu desteklenmiyor. Numarayla web taslagi acabilirim."
                )
            raise invalid_argument("WhatsApp mesaji icin kisi adi veya telefon numarasi gerekli.")
        ok, detail = _open_whatsapp_web(normalized_phone, message)
        if not ok:
            raise capability_unavailable("WhatsApp Web guvenli sekilde acilamadi.")
        source_note = " (rehberden bulundu)" if contact_source == "phone_book" else ""
        return (
            f"WhatsApp sohbeti {detail} icinde {resolved_name or f'+{normalized_phone}'}{source_note} icin taslak mesajla acildi. "
    ...
    if app_target in {"auto", "desktop"}:
        if normalized_phone:
            ...
            if ok:
                ...
                if is_permission_detail(send_detail):
                    raise permission_required(
                        "WhatsApp Desktop otomasyonu icin erisilebilirlik izni gerekiyor."
                    )
                raise capability_unavailable("WhatsApp Desktop otomatik gonderimi guvenli sekilde tamamlanamadi.")
            if app_target == "desktop" and not resolved_name:
                raise capability_unavailable("WhatsApp Desktop guvenli sekilde acilamadi.")
        ...
        if resolved_name:
            ...
            if app_target == "desktop":
                if is_permission_detail(detail):
                    raise permission_required(
                        "WhatsApp Desktop kisi arama otomasyonu icin erisilebilirlik izni gerekiyor."
                    )
                raise capability_unavailable("WhatsApp Desktop kisi arama otomasyonu guvenli sekilde tamamlanamadi.")

    if not normalized_phone:
        if resolved_name:
            raise capability_unavailable(
                f"{resolved_name} icin kayitli bir telefon numarasi bulunamadi. Once kisiyi numarasiyla kaydet."
            )
        raise invalid_argument("WhatsApp mesaji icin kisi adi veya telefon numarasi gerekli.")
    ...
    if is_permission_detail(send_detail):
        raise permission_required("WhatsApp Web otomasyonu icin erisilebilirlik izni gerekiyor.")
    raise capability_unavailable("WhatsApp Web otomatik gonderimi guvenli sekilde tamamlanamadi.")
