...
def browser_control(action: str, url: str = "", query: str = "") -> str:
    ...
    if normalized_action == "search":
        ...
        return f"'{search_query}' için arama açıldı."
    ...
    if not video_id:
        fallback_url = "https://www.youtube.com/results?search_query=" + urllib.parse.quote(youtube_query)
        _launch_url(fallback_url)
        return f"YouTube'da doğrudan video bulunamadı. Arama sonuçları açıldı: {youtube_query}"

    watch_url = f"https://www.youtube.com/watch?v={video_id}&autoplay=1"
    _launch_url(watch_url)
    return f"YouTube'da oynatılıyor: {youtube_query}"
