"""
Apple Reminders tools for elyan.
...
def _permission_message() -> str:
    return (
        ...
        "'elyan Calendar Helper' uygulamasina izin ver."
    )


def _raise_reminder_error(detail: str, *, prefix: str) -> None:
    safe_detail = str(detail or "").strip()
    if is_permission_detail(safe_detail):
        raise permission_required(_permission_message())
...
def get_reminders(query: str = "upcoming", limit: int = 8, list_name: str = "") -> str:
    ...
    if not ok:
        _raise_reminder_error(raw, prefix="Animsaticilar okunamadi")

    parsed_ok, detail, reminders = _parse_payload(raw)
    if not parsed_ok:
        _raise_reminder_error(detail, prefix="Animsaticilar okunamadi")
...
def add_reminder(
    ...
    if not title or not title.strip():
        raise invalid_argument("Animsatici basligi bos olamaz.")
    ...
    if not ok:
        _raise_reminder_error(raw, prefix="Animsatici eklenemedi")

    parsed_ok, detail, created = _parse_created_payload(raw)
    if not parsed_ok:
        _raise_reminder_error(detail, prefix="Animsatici eklenemedi")
